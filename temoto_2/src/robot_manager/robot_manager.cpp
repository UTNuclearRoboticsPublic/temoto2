...
RobotManager::RobotManager() : resource_manager_(srv_name::MANAGER, this)
{
  ...
  // Fire up servers
  server_load_ =
      nh_.advertiseService(robot_manager::srv_name::SERVER_LOAD, &RobotManager::loadCb, this);
  server_plan_ =
...
bool RobotManager::loadCb(temoto_2::RobotLoad::Request& req, temoto_2::RobotLoad::Response& res)
{
  std::string prefix = common::generateLogPrefix(log_subsys_, log_class_, __func__);
  ...
  if (active_robot_)
  {
    ...
    {
      TEMOTO_DEBUG("%s The requested robot '%s' is already loaded.", prefix.c_str(), req.robot_name.c_str());
      res.code = 0;
      res.message = "The requested robot is already loaded.";
      return true;
    }
