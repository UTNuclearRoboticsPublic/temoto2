...
public:
    ...
    , owner_(owner)
  {
    ...
    ROS_DEBUG_NAMED(log_name_, "%s Created ResourceClient %s", prefix.c_str(), name_.c_str());
  }

  ~ResourceClient()
  {
    std::string prefix = common::generateLogPrefix(owner_->getName(), log_name_, "");
    ROS_DEBUG_NAMED(log_name_, "%s Destroyed ResourceClient %s", prefix.c_str(), name_.c_str());
  }

  bool call(ServiceType& msg)
  {
    std::string prefix = common::generateLogPrefix(owner_->getName(), log_name_, "");
    //ROS_DEBUG_NAMED(log_name_, "%s", prefix.c_str());
    ...
    if (q_it == queries_.end())
    {
      ...
      ROS_DEBUG_NAMED(log_name_, "%s New query, performing external call to %s", prefix.c_str(),
               service_client_.getService().c_str());
      if (service_client_.call(msg))
      {
        ROS_DEBUG_NAMED(log_name_, "%s Service call was sucessful. ext id: %ld", prefix.c_str(),
      ...
      {
        ROS_ERROR_NAMED(log_name_, "%s Service call to %s returned false.", prefix.c_str(), service_client_.getService().c_str());
    ...
    else
    {
      ...
      ROS_DEBUG_NAMED(log_name_, "%s Existing request, using stored response", prefix.c_str());
  ...
  {
    std::string prefix = common::generateLogPrefix(owner_->getName(), log_name_, "");
    ROS_DEBUG_NAMED(log_name_, "%s ResourceClient %s is unloading %lu queries.", prefix.c_str(), name_.c_str(), queries_.size());
    for (auto& q : queries_)
    {
  ...
  {
    std::string prefix = common::generateLogPrefix(owner_->getName(), log_name_, "");
    ROS_DEBUG_NAMED(log_name_, "%s Unloading resource with id:%d", prefix.c_str(), resource_id);
    // search for given resource id to unload
    auto q_it = std::find_if(queries_.begin(), queries_.end(),
                             [&](const ClientQuery<ServiceType, Owner>& q) -> bool {
                               return q.internalResourceExists(resource_id);
  ...
  void sendUnloadRequest(temoto_id::ID ext_resource_id)
  {
    ...
    ROS_DEBUG_NAMED(log_name_, "%s Sending unload to %s", prefix.c_str(),
             service_client_unload_.getService().c_str());
    if (!service_client_unload_.call(unload_msg))
    {
      ROS_ERROR_NAMED(log_name_, "%s Call to %s failed.", prefix.c_str(),
                service_client_unload_.getService().c_str());
    }
  }
  ...
  getInternalResources(temoto_id::ID ext_resource_id) const
  {
    ...
    {
      ROS_ERROR_NAMED(log_name_, "%s No query with given resource_id was not found!", prefix.c_str());
    }
  ...
  {
    std::string prefix = common::generateLogPrefix(owner_->getName(), log_name_, "");
    ROS_DEBUG_NAMED(log_name_, "%s  Client name: %s", prefix.c_str(), name_.c_str());
    ROS_DEBUG_NAMED(log_name_, "%s  Ext server name: %s",prefix.c_str(), ext_server_name_.c_str());
    ROS_DEBUG_NAMED(log_name_, "%s  Ext RM name: %s",prefix.c_str(), ext_resource_manager_name_.c_str());
    for(auto& q : queries_)
    {
      q.debug();
    }
