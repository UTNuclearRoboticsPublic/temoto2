#ifndef TEMOTO_SENSOR_MANAGER__SENSOR_MANAGER_INTERFACE_H
#define TEMOTO_SENSOR_MANAGER__SENSOR_MANAGER_INTERFACE_H
...
/**
 * @brief The SensorTopicsReq class
 */
...
/**
 * @brief The SensorTopicsRes class
 */
...
public:
                            ...
                            , const SensorTopicsReq& topics)
  {
    ...
    temoto_sensor_manager::LoadSensor srv_msg;
    ...
    try
    {
      resource_manager_->template call<temoto_sensor_manager::LoadSensor>(sensor_manager::srv_name::MANAGER,
  ...
  void stopSensor(std::string sensor_type, std::string package_name, std::string ros_program_name)
  {
    ...
    temoto_sensor_manager::LoadSensor::Request req;
    ...
    auto found_sensor_it = std::find_if(
        ...
        [&](const temoto_sensor_manager::LoadSensor& srv_msg) -> bool{ return srv_msg.request == req; });
  ...
  void statusInfoCb(temoto_2::ResourceStatus& srv)
  {
    ...
    if (srv.request.status_code == temoto_core::rmp::status_codes::FAILED)
    {
      ...
      auto sens_it = std::find_if(allocated_sensors_.begin(), allocated_sensors_.end(),
                                  [&](const temoto_sensor_manager::LoadSensor& sens) -> bool {
...
private:
  std::string name_;
  std::vector<temoto_sensor_manager::LoadSensor> allocated_sensors_;
